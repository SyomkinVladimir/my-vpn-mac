...
# --- Копируем sing-box в ~/.myvpn при первом запуске ---
def setup_singbox():
    try:
...
# --- Системный прокси (только для режима Системный прокси) ---
    ...
    config = {
        ...
        "dns": {
            ...
            "rules": [
                ...
                {"query_type": ["A", "AAAA"], "server": "fake-dns"}
            ],
            "fakeip": {
                "enabled": True,
        ...
        "outbounds": [
            vless_outbound,
            {"type": "direct", "tag": "direct-out", "tcp_fast_open": True},
...
# --- Мониторинг процесса и авто-восстановление ---
def monitor_process(process, vless_link, mode, log_callback, on_crash_callback, on_recover_callback):
    ...
    if not is_manually_stopped:
        ...
        if current_retries < MAX_RETRIES:
            current_retries += 1
            if log_callback:
                log_callback(f"Попытка восстановления {current_retries}/{MAX_RETRIES}...")
            time.sleep(1.5)

            result = start_vpn(
                vless_link, mode,
                log_callback, on_crash_callback, on_recover_callback,
                is_retry=True
            )
            if result == "успех":
                if on_recover_callback:
                    on_recover_callback(mode)
            else:
                lock_network()
                if on_crash_callback:
                    on_crash_callback(result)
        else:
            lock_network()
            if on_crash_callback:
